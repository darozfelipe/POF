...
#54001 e 54002 e 55064 INSS - aposentadoria/pensao, 
#54003 e 54004 aposentadoria/pensao regime proprio
#54008 aluguel/arrendamento
#54010 bolsa família
#55007 venda automovel
#55008 venda imovel
#55014 emprestimo/penhora

rend_outr <- rend_outr %>%
    mutate(bcat = ifelse(grepl(paste(c('54001', '54003', '54002', '54008', '55014', '55008', '55007', '54004', '54010', '55064'), collapse = "|"), codeIBGE2), desc_rend, "rend_outr")) %>%
    mutate(bcat = ifelse(grepl(paste(c('54001', '55064', '54002'), collapse = "|"), codeIBGE2), "rend_outr_aposentadoria/pensao", bcat)) %>%
    mutate(bcat = ifelse(grepl(paste(c('54003', '54004'), collapse = "|"), codeIBGE2), "rend_outr_aposentadoria/pensao regime proprio", bcat)) %>%
    mutate(bcat = ifelse(grepl(paste(c('54008'), collapse = "|"), codeIBGE2), "rend_outr_aluguel/arrendamento", bcat)) %>%
    mutate(bcat = ifelse(grepl(paste(c('55014'), collapse = "|"), codeIBGE2), "rend_outr_emprestimo/penhora", bcat)) %>%
    mutate(bcat = ifelse(grepl(paste(c('54010'), collapse = "|"), codeIBGE2), "rend_outr_bolsa", bcat)) %>%
    mutate(bcat = ifelse(grepl(paste(c('55007', '55008'), collapse = "|"), codeIBGE2), "rend_outr_automov/imov", bcat)) 
...
rm(rend_outr, rend_trab, ded_ir, ded_outr, ded_outr2, ded_prev)
 ...
 # preenchendo NAs
df_rend[is.na(df_rend)] <- 0

##### Agrupando por UC
# Primeiro vou descondiderar categorias de variacao patrimonial
rend_UC <- df_rend %>%
    select(UniConsumoID, fator_exp_2, valor, bcat) %>%
  group_by(UniConsumoID, bcat) %>%
    summarise(valor = sum(valor),
              fator_exp_2 = min(fator_exp_2)) %>%
    pivot_wider(names_from = bcat, values_from = valor)
...
rend_UC <- rend_UC %>%
  ...
  mutate(rend_pat = ifelse((patr_pos - patr_neg) < 0, 0, (patr_pos - patr_neg))) %>%
...
rend_UC$UniConsumoID <- as.character(rend_UC$UniConsumoID)
# renda total e deducao total
rend_UC <- rend_UC %>%
    group_by(UniConsumoID) %>%
  mutate(rend_tot = sum(rend_outr + `rend_outr_aluguel/arrendamento` + `rend_outr_automov/imov` + rend_trab + rend_outr_bolsa + `rend_outr_aposentadoria/pensao` + `rend_outr_emprestimo/penhora` + `rend_outr_aposentadoria/pensao regime proprio` + rend_pat),
         ded_tot = sum(ded_ir + ded_outr + ded_outr2 + ded_prev))
...
rend_UC <- rend_UC %>%
  mutate_at(.funs = list(percap = ~./quant_mor), vars(matches("rend_"))) %>%
  mutate_at(.funs = list(percap = ~./quant_mor), vars(matches("ded_")))


# calculando percentis de renda
df_faixa <- data.frame("perc" = seq(0,1,.01))
df_faixa$rend_pc_faixa <- Hmisc::wtd.quantile(rend_UC$rend_tot_percap, weights = rend_UC$fator_exp_2, probs = seq(0,1,.01))


### PRECISO CRIAR CATEGORIAS NO DF FINAL PARA CADA FAIXA DE RENDA
rend_UC$faixa <- NA
for (i in 1:nrow(rend_UC)) {
    rend_UC$faixa[i] <- (as.numeric(findInterval(rend_UC$rend_tot_percap[i],df_faixa[,2], rightmost.closed = TRUE, left.open = TRUE, all.inside = TRUE)))
}
...
df_final <- rend_UC %>%
    group_by(faixa) %>%
    summarise(rend_tot = sum(rend_tot_percap),
              rend_trab = sum(rend_trab_percap),
              rend_pat = sum(rend_pat_percap),
              rend_outr = sum(rend_outr_percap),
              ded_tot = sum(ded_tot_percap),
              ded_ir = sum(ded_ir_percap),
              ded_prev = sum(ded_prev_percap),
              ded_outr = sum(ded_outr_percap),
              ded_outr2 = sum(ded_outr2_percap),
              rend_outr_aluguel = sum(`rend_outr_aluguel/arrendamento_percap`),
              rend_outr_automv_imov = sum(`rend_outr_automov/imov_percap`),
              rend_outr_bolsafam = sum(rend_outr_bolsa_percap),
              rend_outr_aposentadoria_pensao = sum(`rend_outr_aposentadoria/pensao_percap`),
              rend_outr_emprest = sum(`rend_outr_emprestimo/penhora_percap`),
              rend_outr_aposent_pensao_regimeprop = sum(`rend_outr_aposentadoria/pensao regime proprio_percap`))
...
df_final[,2:16] <- round_df(df_final[,2:16],2)
...
df_rend <- df_final[,c(1,3,4,5,11:16)]
df_ded <- df_final[,c(1,7:10)]
df_tot <- df_final[,c(1,2,6)]

# antes: transformando colunas em linhas
df_rend <- pivot_longer(df_rend, 
                  cols = rend_trab:rend_outr_aposent_pensao_regimeprop)
